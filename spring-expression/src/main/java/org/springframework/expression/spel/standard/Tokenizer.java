...
class Tokenizer {
	...
	private static final byte IS_DIGIT = 0x01;//1

	private static final byte IS_HEXDIGIT = 0x02;//2

	private static final byte IS_ALPHA = 0x04;//4
	...
	//the expressionString that uncomplie
	String expressionString;

	//the char array that can be
	char[] toProcess;

	//current pointer postion.
	int pos;
	...
	public void process() {
		while (this.pos < this.max) {
			//peek th charater of current.
			char ch = this.toProcess[this.pos];
			if (isAlphabetic(ch)) {
				//if th ch is a Alphabetic then do  lexIdentifier
				lexIdentifier();
			}
			else {
				switch (ch) {
